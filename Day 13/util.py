...
def _recommendation(prediction, raw_risk_score):
    """Give a recommendation based on the model's prediction and raw risk signal."""
    if prediction == 1:
        return (
            "High-risk claim detected. Broker should verify police report, "
            "witness availability, claim timing, past claim history, and all "
            "supporting evidence before submission."
        )
    if raw_risk_score is not None and raw_risk_score >= 0.08:
        return (
            "Some risk signals present. Broker should review claim documents "
            "and confirm that evidence is complete before submission."
        )
    return "Low-risk claim. Evidence packet can proceed with standard broker review."


def _scale_risk_score(raw_score, prediction):
    """
    Scale the raw Decision Tree probability to a meaningful display range.

    The Decision Tree outputs only discrete probability values.
    Low-risk verdicts (prediction=0) have raw_score in [0, 0.25].
    High-risk verdicts (prediction=1) have raw_score in [0.5, 1.0].

    We map:
      prediction=0 (Low Risk) : raw [0, 0.25]  -> display [3%,  43%]
      prediction=1 (High Risk): raw [0.5, 1.0] -> display [65%, 100%]

    This ensures the gauge needle clearly reflects both the verdict AND
    the degree of suspicion within that verdict.
    """
    if raw_score is None:
        return 0.03 if prediction == 0 else 0.75

    if prediction == 0:
        clamped = min(max(float(raw_score), 0.0), 0.25)
        return round(0.03 + (clamped / 0.25) * 0.40, 4)
    else:
        clamped = min(max(float(raw_score), 0.5), 1.0)
        return round(0.65 + ((clamped - 0.5) / 0.5) * 0.35, 4)
...
def predict_claim_risk(raw_input):
    ...
    # Use the model's own verdict — never override with manual thresholds
    prediction = int(_model.predict(feature_frame)[0])
    raw_risk_score = 0.0

    if hasattr(_model, "predict_proba"):
        raw_risk_score = float(_model.predict_proba(feature_frame)[0][1])
    elif hasattr(_model, "decision_function"):
        raw_risk_score = float(_model.decision_function(feature_frame)[0])

    risk_label = (
        "High-risk / suspicious claim" if prediction == 1
        else "Low-risk / genuine claim"
    )

    # Scale for gauge display: Low stays 3-43%, High stays 65-100%
    display_score = _scale_risk_score(raw_risk_score, prediction)
    ...
    return {
        ...
        "input_used": claim,
        "recommendation": _recommendation(prediction, raw_risk_score),
    }
